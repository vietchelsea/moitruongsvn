	...
	<div class="twelve columns feature-three">
		<div class="slide-large-wrapper header-slider2">
			<div class="owl_slider slider-large owl-carousel">
				<?php $cats="" ; $number_slider=o f_get_option( 'slider_number'); $category_slider=o f_get_option( 'slider_category_select'); $cats='' ; if(!empty($category_slider)) { foreach($category_slider as $key=>$value) { if($value == 1) { $cats[] = $key; } } } $post_array_slider = array( 'showposts' => $number_slider, 'category__in' => $cats, 'ignore_sticky_posts' => 1 ); $jellywp_widget_slider = new WP_Query($post_array_slider); $i=0; while ($jellywp_widget_slider->have_posts()) { $jellywp_widget_slider->the_post(); $i++; $post_id = get_the_ID(); //get all post categories $categories = get_the_category(get_the_ID()); ?>
...
<?php if ( has_post_thumbnail()) {the_post_thumbnail('slider-normal');}
 ...
 <?php echo total_score_post_front(get_the_ID());?> 
		...
		<?php $cats_right="" ; $category_slider_right=o f_get_option( 'category_on_right_slider'); if (of_get_option( 'number_offset_post_right')){$number_offset_post_right=o f_get_option( 'number_offset_post_right');}else{$number_offset_post_right=0 ;} $cats_right='' ; if(!empty($category_slider_right)) { foreach($category_slider_right as $key=>$value) { if($value == 1) { $cats_right[] = $key; } } } $post_array_right = array( 'showposts' => 3, 'category__in' => $cats_right, 'ignore_sticky_posts' => 1, 'offset' => $number_offset_post_right ); $jellywp_widget_right = new WP_Query($post_array_right); $i=0; while ($jellywp_widget_right->have_posts()) { $jellywp_widget_right->the_post(); $i++; $post_id = get_the_ID(); //get all post categories $categories = get_the_category(get_the_ID()); ?>
...
<?php if ( has_post_thumbnail()) {the_post_thumbnail('feature-grid');}
		...
		<div class="grid_3 grid caption_header">
			...
			<div class="link-more"><a href="<?php the_permalink(); ?>">&nbsp;</a>
			</div>
		</div>
		<?php }}?>
	</div>
...
<div class="row">
	<div class="twelve columns carousel_header_wrapper">
		...
		<div class="owl_carousel carousel_header">
			<?php $category_carousel_post="" ; $number_of_carousel=o f_get_option( 'number_carousel'); $category_carousel=o f_get_option( 'carousel_post'); if (of_get_option( 'number_offset_carousel_post')){$number_offset_carousel_post=o f_get_option( 'number_offset_carousel_post');}else{$number_offset_carousel_post=0 ;} if(!empty($category_carousel)) { foreach($category_carousel as $key=>$value) { if($value == 1) { $category_carousel_post[] = $key; } } } $post_array_carousel = array( 'showposts' => $number_of_carousel, 'category__in' => $category_carousel_post, 'ignore_sticky_posts' => 1, 'offset' => $number_offset_carousel_post ); $jellywp_widget_carousel = new WP_Query($post_array_carousel); $i=0; while ($jellywp_widget_carousel->have_posts()) { $jellywp_widget_carousel->the_post(); $i++; $post_id = get_the_ID(); //get all post categories $categories = get_the_category(get_the_ID()); ?>
